<template>
    <div v-if="!loading" :style="'min-height: '+clientHeight/10*9+'px'">
        <div style="font-size: 18px; text-align: center; padding: 30px 0;"><h2>编辑产品</h2></div>
        ...
        <mt-button size="large" style="margin-top: 10px; background-color: #06AD56;color: white" @click.native="saveItem" :disabled="itemData.name===''">保存修改</mt-button>
    ...
    export default {
        ...
        methods: {
            ...
            saveItem () {
                ...
                that.$http.post(that.$store.state.api + '/item/' + that.id, sendData)
                    .then(data => {
                        const Data = data.data.data
                        console.log(Data)
                        that.addOK()
